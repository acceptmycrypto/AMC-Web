...
class SearchDeals extends Component {
  ...
  goToSearchPage = page => {
    if (this.props.pageType === "search") {
      ...
      this.props.history.push(
        "/Search?term=" + this.props.searchTerm + "&page=" + parseInt(page)
      );
    } else if (this.props.pageType === "category") {
      this.props.categoryDeals(this.props.categoryTerm, parseInt(page));
      this.props.history.push(
        "/category?term=" + this.props.categoryTerm + "&page=" + parseInt(page)
      );
    }
  };
  ...
  render() {
    ...
    let {
      ...
      searchTerm,
      categoryTerm
    } = this.props; //does this need to be const?? i changed it to let so line 56 will work
      ...
      <div>
        <Layout>
          ...
          <div className="search-deals-wrapper">
            {deals != undefined && deals.length > 0 && currentTerm !== "" && (
                ...
                <div className="page_NavContent">
                  <span>
                    <button
                      className={
                        "scroll_Button " +
                        ("button_Hide_" + (parseInt(currentPage) == 1))
                      }
                      onClick={() => this.goToSearchPage(1)}
                    >
                      First Page
                    </button>
                  </span>
                  <span>
                    <button
                      className={
                        "page_Number " +
                        ("button_Hide_" + (parseInt(currentPage) - 2 < 1))
                      }
                      onClick={() =>
                  ...
                  <span>
                    <button
                      className={
                        "page_Number " +
                        ("button_Hide_" + (parseInt(currentPage) - 1 < 1))
                      }
                      onClick={() =>
                  ...
                  <span>
                    <button className="page_Number ">{currentPage}</button>
                  </span>
                  <span>
                    <button
                      className={
                ...
                {deals != undefined &&
                  ...
                  deals.map(deal => (
                    <div key={deal.id} className="category_item">
                      <Link
                        to={`/feed/deals/${deal.id}/${deal.deal_name}`}
                      ...
                      >
                        <div className="category-info">
                          ...
                          <div className="mt-1 text-center mr-1 ml-1">
                            {mobileScreenSize.matches
                              ? handleLongDescription(deal.deal_name, 20, 20)
                              : handleLongDescription(deal.deal_name, 50, 50)}
                          </div>
                          ...
                          <div className="text-center mr-1 ml-1 mob-hidden">
                            <small>
                              Offered by: {deal.venue_name || deal.seller_name}
                            </small>
                          </div>
                        </div>
                        ...
                        <div className="deal-price">
                          <div className="price-differ">
                            ...
                            <div className="d-flex flex-column text-center justify-content-center">
                              ...
                              <strong className="pay_in_crypto">
                                ${deal.pay_in_crypto.toFixed(2)}
                              </strong>
                              <small className="pay_in_crypto discount">
            ...
            {deals != undefined && deals.length > 0 && currentTerm != "" && (
              <div>
                <div className="page_Nav page_pagination_bottom">
                  <div className="page_NavContent">
                    <span>
                      <button
                        className={
...
const mapStateToProps = state => ({
  ...
  categoriesDealsOnMobile: state.Category.categoriesDealsOnMobile,
...
const matchDispatchToProps = dispatch => {
  return bindActionCreators(
    { searchDeals, categoryDeals, resetDealitemState, _isLoggedIn },
    dispatch
  );
};
