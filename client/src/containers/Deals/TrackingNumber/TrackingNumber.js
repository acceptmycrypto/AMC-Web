...
import { updateTrackingNumber, editTrackingNumber } from "../../../actions/dealsActions";
...
class TrackingNumber extends Component {
    ...
    render() {
        ...
        let{editTrackingNumber, trackingNumber, updateTrackingNumber} = this.props;
        console.log(txn_id);
        return (
            <div className="pt-5">
                <Layout >
                    {deal_name !== undefined && <div className="container mx-0 text-center">
                        ...
                        <input
                            ...
                            style={{width:"300px"}}
                        />
                        <div className="btn btn-info ml-4" onClick={async (event)=>{await updateTrackingNumber(localStorage.getItem('token'), txn_id, trackingNumber); await this.props.history.push(`/`)}}>Submit</div>
                    </div>}
                </Layout >
...
const matchDispatchToProps = dispatch => {
    ...
    return bindActionCreators({updateTrackingNumber, editTrackingNumber, _isLoggedIn}, dispatch);
