...
router.get("/community", function(req, res) {
  connection.query("SELECT * FROM crypto_info", (err, data) => {
    if (err) {
      console.log(err);
    } else {
      console.log(data);
...
assembleComments = data => {
  ...
  let id,
    ...
    x;
  for (x of data) {
    //cycle thru all returned comments
    ({
      id,
      user_id,
      crypto_id,
    ...
    if (x.comment_parent_id == null) {
      //if comment does not have parent id, this comment is a parent itself, therefore append info from x to allComments
      allComments = [
        ...allComments,
        {
          id,
      ...
      //if comment has parent id, this comment is a child
...
router.post("/crypto/submit-comment", verifyToken, function(req, res) {
  ...
  body = arr.map(x => {
    ...
    let res2 = arr2.map(y => {
      if (regExp.test(y)) {
        if (url == "") {
          //ideally the comment will only have one url in it, but in case there are multiple, store the first url
          url = y;
        }
      ...
      function(err, insertData) {
        if (err) {
          ...
          if (comment_parent_id == 0) {
            //this means the submitted comment is a parent comment, can go straight to getAllComments
            getAllComments(res, crypto_id);
          } else {
            //this means the submitted comment is a child comment, so must also update parents_children table
            connection.query(
